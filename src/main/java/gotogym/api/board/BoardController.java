...
import gotogym.models.board.*;
import gotogym.models.board.config.BoardConfigInfoService;
import gotogym.models.board.config.BoardNotAllowAccessException;
...
public class BoardController {
    ...
    public String write(@PathVariable String bId, @ModelAttribute BoardForm boardForm, Model model) {
        ...
        if (customJwtFilter.isUserLoggedIn()) {
            boardForm.setWriter(customJwtFilter.getMember().getName());
        }
    ...
    public String update(@PathVariable Long id, Model model) {
        PostBoardData boardData = infoService.get(id, "update");
    ...
    public String save(@Valid BoardForm boardForm, Errors errors, Model model) {
        ...
        if (id != null) {
            mode = "update";
            PostBoardData boardData = infoService.get(id);
            board = boardData.getBoard();
            if (boardData.getMember() == null) {
                board.setGuest(true);
            } else {
                boardForm.setUserNo(boardData.getMember().getSeq());
            }
     ...
     * @param boardData
     */
    public void updateDeletePossibleCheck(PostBoardData boardData, String mode) {
        mode = mode == null ? "board" : mode;
        if (customJwtFilter.isUserLoggedIn()) { // 관리자는 무조건 가능
            return;
        }
        ...
        if (member == null) { // 비회원일때는 비밀번호 검증이 되었는지 체크, 안되어 있다면 비밀번호 확인 페이지 이동
            ...
            if (customJwtFilter.isUserLoggedIn()
                    && customJwtFilter.getMember().getUserNo() != boardData.getMember().getSeq()) {
    ...
    public void updateDeletePossibleCheck(PostBoardData boardData) {
        updateDeletePossibleCheck(boardData, null);
    }
