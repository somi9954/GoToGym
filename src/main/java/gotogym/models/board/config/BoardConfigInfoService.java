...
public class BoardConfigInfoService {
    ...
    public void accessCheck(Board board, String location) {
         ...
         */
        if (!board.isUse() && !customJwtFilter.isUserAdmin()) {
            throw new BoardNotAllowAccessException();
        }

        MemberType role = MemberType.All;
        if (location.equals("list")) { // 목록 접근 권한
            ...
            /** 비회원 게시글 여부 */
            if(!customJwtFilter.isUserLoggedIn()) board.setGuest(true);
        ...
        if ((role == MemberType.USER && !customJwtFilter.isUserLoggedIn())
            || (role == MemberType.ADMIN && !customJwtFilter.isUserAdmin())) {
            throw new BoardNotAllowAccessException();
        }
    }
}
